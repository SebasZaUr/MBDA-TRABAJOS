...
/*GC OFERTA*/
INSERT INTO Ofertas VALUES(1,'1-ene-2022','Venta de apartamento peque�o','cl4#12-4','A',90,'T',20000000,'T','re','D',1,3);
INSERT INTO Ofertas VALUES(2,'1-ene-2022','Venta de apartamento peque�o','cl3#12-4','A',90,'T',20000000,'T','re','D',1,3);
INSERT INTO Ofertas VALUES(3,'1-ene-2022','Venta de apartamento peque�o','cr4#1-4','A',90,'T',20000000,'T','re','D',1,3);
...
/*GC OFERTA*/
INSERT INTO Ofertas VALUES(1,'1-ene-2022','Venta de apartamento peque�o','cl4#12-4','A',90,'F',20000000,'T','re.XML','D',1,'U-12');
...
INSERT INTO Ofertas VALUES(5,'1-ene-2022','arriendo de apartamento peque�o','cl2#12-4','A',90,'F',20000000,'T','re.XML','D',5,'U-1');
INSERT INTO Ofertas VALUES(6,'1-ene-2022','Venta de casa peque�a','cr4#1-7','C',90,'T',20000000,'T','re.XML','D',6,'U-12');
INSERT INTO Ofertas VALUES(7,'1-ene-2022','Venta de apartamento ','cl4#12-12','A',90,'T',20000000,'T','re.XML','D',7,'U-11');
INSERT INTO Ofertas VALUES(8,'1-ene-2022','Venta de apartamento peque�a','cl43#12-4','A',90,'T',20000000,'T','re.XML','D',8,'U-11');
INSERT INTO Ofertas VALUES(9,'1-ene-2022','Venta de apartamento peque�a','cr14#1-4','A',90,'T',20000000,'T','re.XML','D',9,'U-14');
INSERT INTO Fotografias VALUES('foto fachada','http:fotoFachada.com','foto fachada de la casa');
INSERT INTO Fotografias VALUES('foto Cocina','http:fotoParqueadero.com','foto cocina a gas y espacio');
INSERT INTO Fotografias VALUES('Parqueadero','http:fotoFachada.com','foto parqueadero de la casa donde puede guardar dos carros');
INSERT INTO OpcionesCredito VALUES(1,60,100000);
...
SELECT numero,descripcion,tamaño,habitada,costo
FROM ofertas
WHERE tipoVivienda LIKE 'C';

/*add oferta*/
CREATE TRIGGER TR_OFERTAS_FECHA_BI
...
CREATE TRIGGER TR_OFERTAS_HABITADA_BF
BEFORE INSERT ON ofertas
FOR EACH ROW
BEGIN
IF Ofertas.habitada IS NULL
THEN
Ofertas.habitada := 'F';
Ofertas.proyectoVivienda := 'T';
End If;
END TR_OFERTAS_HABITADA_BF;
/

CREATE TRIGGER TR_CREDITO_BF
BEFORE INSERT ON OpcionesCredito
FOR EACH ROW
BEGIN
new.plazo := 12; 
new.valorMensual := (ofertas.costo +(ofertas.costo*0.1))/12;
END TR_CREDITO_BD;
/
 
 /*MO*/
 /*solo se actualiza los anexos*/
 CREATE TRIGGER TR_MODIFIED_BU
BEFORE UPDATE ON Ofertas
FOR EACH ROW
BEGIN
   IF NOT updating('anexos') THEN
    raise_application_error(-20001,'No se puede modificar este campo');
   END IF;
 END TR_MODIFIED_BU;
 /
/*modificar fotografias*/
 CREATE TRIGGER TR_MODIFIED_PHOTO_BU
BEFORE UPDATE ON FotografiasOfertas
FOR EACH ROW
BEGIN
   IF NOT updating('Fotografias_nombre') THEN
    raise_application_error(-20001,'No se puede modificar este campo');
   END IF;
 END TR_MODIFIED_PHOTO_BU;
 /

/*eliminar oferta si es la iltima*/
CREATE TRIGGER TR_DELETE_OFERTA_BF
BEFORE DELETE ON Ofertas
FOR EACH ROW
BEGIN 
DELETE FROM Ofertas WHERE Ofertas.numero = (SELECT max(numero) FROM Ofertas);
    
END TR_DELETE_OFERTA_BF;
/

/*Delete OpcionCredito*/
CREATE TRIGGER TR_DELETE_OPCIONC_BF
BEFORE DELETE ON OpcionesCredito
FOR EACH ROW
BEGIN
IF OpcionesCredito.valorMensual * OpcionesCredito.plazo >= Ofertas.costo THEN
DELETE OpcionesCredito;
END IF; 
END TR_DELETE_OPCIONC_BF;
/

CREATE TRIGGER TR_UPDATE_COND_BF
